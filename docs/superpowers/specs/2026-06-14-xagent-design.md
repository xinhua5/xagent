...
| 转换 | 触发条件 |
|------|---------|
| OPEN → CONFLICT | 检测到观点冲突或时空冲突 |
| CONFLICT → STALLED | 关键人 30 分钟未回复（沉默冲突），或冲突双方均不再发言 |
| STALLED → RESOLVED | Agent 介入调解，提出折中方案并被接受 |
| STALLED → ESCALATED | 停滞超过 60 分钟仍未解决，升级到更高决策权的人 |
| STALLED → CLOSED | 两次升级后仍无回应，按默认策略关闭 |
| 任意 → CLOSED | 手动关闭，或讨论主题已过时（Feature 已完成/取消） |

**STALLED 与 CONFIRM 超时的区别**：
- STALLED 是讨论线程的状态——多人讨论卡住了
- CONFIRM 超时是单个确认请求的状态——一个人没回复
- 两者独立运行，可能同时发生（讨论卡住了，其中某个确认请求也超时了）
- STALLED 不触发 Safety Model 审查——讨论是人的事，AI 只负责调解和升级
...
当多个 sub-agent 并行执行时，每个 sub-agent 在自己的 overlay 层上工作：

```
真实文件系统 (base)
    │
    ├── Overlay A (sub-agent 1: task_042 创建 auth.py)
    │     └── 对主 Agent 和 sub-agent 1 可见 auth.py
    │
    ├── Overlay B (sub-agent 2: task_043 创建 models.py)
    │     └── 对主 Agent 和 sub-agent 2 可见 models.py
    │
    └── Overlay C (sub-agent 3: task_044 修改 config.py)
          └── 对主 Agent 和 sub-agent 3 可见 config.py 的修改
```
...
| 规则 | 说明 |
|------|------|
| 每个 sub-agent 独享 overlay | 看不到其他 sub-agent 的未提交变更 |
| 主 Agent 合并视图 | 主 Agent 可以看到所有 overlay 的叠加效果 |
| 同文件冲突检测 | 如果两个 sub-agent 同时修改同一个文件 → 后提交的检测到冲突 → 主 Agent 仲裁 |
| Overlay 提交 | sub-agent 完成后，overlay delta 合并到 base → 其他 sub-agent 立即可见 |
| Overlay 丢弃 | sub-agent 失败 → overlay delta 直接丢弃 → 无任何残留，不影响其他 sub-agent |
| 合并顺序 | 先完成先合并。如果顺序有关系，通过任务的 `depends_on` 显式声明 |
...
cost:
  ...
  # BRAIN Tier 每日配额（含 Safety Model）
  brain_daily_limit:
    task_decision: 20       # 任务决策类 BRAIN 调用
    safety_review: 50       # Safety Model 审查调用（比任务决策宽松）
    total: 70               # 总上限

  # Safety Model 配额超限行为（与任务决策超限不同）
  safety_model_on_exceeded:
    action: "downgrade_to_rule"   # 不用 AI 审查，降级为规则引擎
    rule_engine_behavior:         # 规则引擎的默认行为
      auto_level_ops: "deny"      # AUTO 操作在无 Safety Model 时 → 拒绝
      auto_plus_level_ops: "escalate"  # AUTO+ 操作在无 Safety Model 时 → 升级人工
    notify: true                  # 通知团队 Safety Model 配额用尽
    reset: "daily"                # 每天 00:00 重置
```
...
Safety Model 配额用尽
       │
       ▼
AUTO 操作 → 规则引擎 → 默认 DENY（不执行）
                        → 可配置: 项目 owner 可临时开放

AUTO+ 操作 → 规则引擎 → 默认 ESCALATE（升级人工确认）
                         → 相当于临时降级为 CONFIRM 级别

CONFIRM 操作 → 不受影响（本来就走人工）
REJECT 操作 → 不受影响（本来就不走任何审查）
```
...
│                                              │
│  * BRAIN (安全) = Safety Model 审查消耗，     │
│    与 BRAIN (任务) 同模型但按用途逻辑分账      │
└─────────────────────────────────────────────┘
```
...
| Cache 命中率 | >80% | 减少重复计费 |
| BRAIN 调用/天（含 Safety Model） | <70 | 其中 Safety Model <50, 任务决策 <20 |
